...
class FatFreeCRM::CustomFieldPair < FatFreeCRM::CustomField
  has_one :pair, class_name: 'FatFreeCRM::CustomFieldPair', foreign_key: 'pair_id', dependent: :destroy # points to 'end'
  ...
  def self.update_pair(params)
    ...
    pair = params.delete('pair')
    base_params = fields.delete_if { |k, _v| !%w(field_group_id label as).include?(k) }
    field1 = FatFreeCRM::CustomFieldPair.find(params['id'])
  ...
  #------------------------------------------------------------------------------
  def paired_with
    pair || FatFreeCRM::CustomFieldPair.where(pair_id: id).first
  end
