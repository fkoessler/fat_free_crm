...
class FatFreeCRM::Account < ActiveRecord::Base
  ...
  has_many :account_contacts, dependent: :destroy, class_name: 'FatFreeCRM::AccountContact'
  has_many :contacts, -> { distinct }, through: :account_contacts, class_name: 'FatFreeCRM::Contact'
  has_many :account_opportunities, dependent: :destroy, class_name: 'FatFreeCRM::AccountOpportunity'
  has_many :opportunities, -> { order("fat_free_crm_opportunities.id DESC").distinct }, through: :account_opportunities, class_name: 'FatFreeCRM::Opportunity'
  has_many :tasks, as: :asset, dependent: :destroy, class_name: 'FatFreeCRM::Task' # , :order => 'created_at DESC'
  has_one :billing_address, -> { where(address_type: "Billing") }, dependent: :destroy, as: :addressable, class_name: "FatFreeCRM::Address"
  has_one :shipping_address, -> { where(address_type: "Shipping") }, dependent: :destroy, as: :addressable, class_name: "FatFreeCRM::Address"
  has_many :addresses, dependent: :destroy, as: :addressable, class_name: "FatFreeCRM::Address" # advanced search uses this
  has_many :emails, as: :mediator, class_name: 'FatFreeCRM::Email'

  serialize :subscribed_users, Set

  accepts_nested_attributes_for :billing_address,  allow_destroy: true, reject_if: proc { |attributes| FatFreeCRM::Address.reject_address(attributes) }
  accepts_nested_attributes_for :shipping_address, allow_destroy: true, reject_if: proc { |attributes| FatFreeCRM::Address.reject_address(attributes) }
  ...
  validates_uniqueness_of :name, scope: :deleted_at, if: -> { FatFreeCRM::Setting.require_unique_account_names }
  # validates_uniqueness_of :clktcode, scope: [:clktsoc]
  ...
  def self.create_or_select_for(model, params)
    if params[:id].present?
      account = FatFreeCRM::Account.find(params[:id])
    else
      account = FatFreeCRM::Account.new(params)
