...
class FatFreeCRM::Lead < ActiveRecord::Base
  ...
  has_one :contact, dependent: :nullify, class_name: 'FatFreeCRM::Contact' # On destroy keep the contact, but nullify its lead_id
  ...
  validates :status, inclusion: { in: proc { FatFreeCRM::Setting.unroll(:lead_status).map { |s| s.last.to_s } } }, allow_blank: true
  ...
  def save_with_permissions(params)
    ...
    if params[:lead][:access] == "Campaign" && campaign # Copy campaign permissions.
      save_with_model_permissions(FatFreeCRM::Campaign.find(campaign_id))
    else
  ...
  def promote(params)
    ...
    account     = FatFreeCRM::Account.create_or_select_for(self, account_params)
    opportunity = FatFreeCRM::Opportunity.create_for(self, account, opportunity_params)
    contact     = FatFreeCRM::Contact.create_for(self, account, opportunity, params)
  ...
  def increment_leads_count
    if campaign_id
      FatFreeCRM::Campaign.increment_counter(:leads_count, campaign_id)
    end
  end
  ...
  def decrement_leads_count
    if campaign_id
      FatFreeCRM::Campaign.decrement_counter(:leads_count, campaign_id)
    end
  end
