...
class FatFreeCRM::Contact < ActiveRecord::Base
  belongs_to :user
  belongs_to :lead, class_name: 'FatFreeCRM::Lead'
  belongs_to :assignee, class_name: 'User', foreign_key: :assigned_to
  belongs_to :reporting_user, class_name: 'User', foreign_key: :reports_to
  has_one :account_contact, dependent: :destroy, class_name: 'FatFreeCRM::AccountContact'
  has_one :account, through: :account_contact, class_name: 'FatFreeCRM::Account'
  has_many :contact_opportunities, dependent: :destroy, class_name: 'FatFreeCRM::ContactOpportunity'
  has_many :opportunities, -> { order("fat_free_crm_opportunities.id DESC").distinct }, through: :contact_opportunities, class_name: 'FatFreeCRM::Opportunity'
  has_many :tasks, as: :asset, dependent: :destroy, class_name: 'FatFreeCRM::Task' # , :order => 'created_at DESC'
  has_one :business_address, -> { where(address_type: "Business") }, dependent: :destroy, as: :addressable, class_name: "FatFreeCRM::Address"
  ...
  accepts_nested_attributes_for :business_address, allow_destroy: true, reject_if: proc { |attributes| FatFreeCRM::Address.reject_address(attributes) }

  scope :created_by,  ->(user) { where(user_id: user.id) }
  scope :assigned_to, ->(user) { where(assigned_to: user.id) }

  scope :text_search, ->(query) {
    t = FatFreeCRM::Contact.arel_table
  ...
  def save_with_account_and_permissions(params)
    save_account(params)
    result = save
    opportunities << FatFreeCRM::Opportunity.find(params[:opportunity]) unless params[:opportunity].blank?
    result
  end
  ...
  def self.create_for(model, account, opportunity, params)
    ...
    contact = FatFreeCRM::Contact.new(attributes)
    ...
    if account.errors.empty? && opportunity.errors.empty?
      ...
      contact.account_contact = FatFreeCRM::AccountContact.new(account: account, contact: contact) unless account.id.blank?
  ...
  def save_account(params)
    ...
    if account_params[:id] == "" || account_params[:name] == ""
      ...
      self.account = FatFreeCRM::Account.create_or_select_for(self, account_params)
    end
  end
