...
class FatFreeCRM::Campaign < ActiveRecord::Base
  ...
  has_many :leads, -> { order "id DESC" }, dependent: :destroy, class_name: 'FatFreeCRM::Lead'
  has_many :opportunities, -> { order "id DESC" }, dependent: :destroy, class_name: 'FatFreeCRM::Opportunity'
  ...
  validates :status, inclusion: { in: proc { FatFreeCRM::Setting.unroll(:campaign_status).map { |s| s.last.to_s } } }, allow_blank: true
