...
class FatFreeCRM::Opportunity < ActiveRecord::Base
  belongs_to :user
  belongs_to :campaign, class_name: 'FatFreeCRM::Campaign'
  belongs_to :assignee, class_name: 'User', foreign_key: :assigned_to
  has_one :account_opportunity, dependent: :destroy, class_name: 'FatFreeCRM::AccountOpportunity'
  has_one :account, through: :account_opportunity, class_name: 'FatFreeCRM::Opportunity'
  has_many :contact_opportunities, dependent: :destroy, class_name: 'FatFreeCRM::ContactOpportunity'
  has_many :contacts, -> { order("fat_free_crm_contacts.id DESC").distinct }, through: :contact_opportunities, class_name: 'FatFreeCRM::Contact'
  ...
  validates :stage, inclusion: { in: proc { FatFreeCRM::Setting.unroll(:opportunity_stage).map { |s| s.last.to_s } } }, allow_blank: true
  ...
  def save_with_account_and_permissions(params)
    ...
    result = save
    contacts << FatFreeCRM::Contact.find(params[:contact]) unless params[:contact].blank?
    result
  end
  ...
  def update_with_account_and_permissions(params)
    if params[:account] && (params[:account][:id] == "" || params[:account][:name] == "")
      ...
      self.account = FatFreeCRM::Account.create_or_select_for(self, params[:account])
    end
  ...
  def self.create_for(model, account, params)
    ...
    if opportunity.name? && account.errors.empty?
      ...
      opportunity.account_opportunity = FatFreeCRM::AccountOpportunity.new(fat_free_crm_account: account, fat_free_crm_opportunity: opportunity) unless account.id.blank?
  ...
  def increment_opportunities_count
    if campaign_id
      FatFreeCRM::Campaign.increment_counter(:opportunities_count, campaign_id)
    end
  end
  ...
  def decrement_opportunities_count
    if campaign_id
      FatFreeCRM::Campaign.decrement_counter(:opportunities_count, campaign_id)
    end
  end
